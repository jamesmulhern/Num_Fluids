...
for j = 2:Nx-1
        m = k + (j-1)*Ny;  
        AP(m)     = kappa*(2*dy/dx+3*dx/dy);
        A(m,m)    = AP(m);
        A(m,m-Ny) = AW(m);
        A(m,m+Ny) = AE(m);
        A(m,m+1)  = AN(m);
        B(m)      = 2*kappa*dx/dy*Ts(j);
end
%
%%
% west boundary
...
k = 1;
m = k + (j-1)*Ny;
AP(m)     = 3*kappa*(dy/dx+dx/dy);
A(m,m)    = AP(m);
A(m,m+Ny) = AE(m);
A(m,m+1)  = AN(m);
B(m)      = 2*kappa*dy/dx*Tw(k) + 2*kappa*dx/dy*Ts(j);
...
A(m,m-Ny) = AW(m);
A(m,m+1 ) = AN(m);
B(m)      = 2*kappa*dy/dx*Te(k) + 2*kappa*dx/dy*Ts(j);
...
A(m,m-Ny) = AW(m);
A(m,m-1 ) = AS(m);
B(m)      = 2*kappa*dy/dx*Te(k) + 2*kappa*dx/dy*Tn(j);
...
A(m,m+Ny) = AE(m);
A(m,m-1 ) = AS(m);
B(m)      = 2*kappa*dy/dx*Tw(k) + 2*kappa*dx/dy*Tn(j);
