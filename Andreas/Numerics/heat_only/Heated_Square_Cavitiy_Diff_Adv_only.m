...
dy = Ly/Ny;
%
[x,y] = meshgrid(0:dx:Lx, 0:dy:Ly); % coordinates of grid points
% cell corners
%
xcorners = 0:dx:Lx;
ycorners = 0:dy:Ly;
...
%
% velocity field at the corners given by BC and is Zero everywhere
%
ux = xcorners;
uy = ycorners;
% ux = xcorners * 0;
...
% % Initial temperature is function of x
% 
% 
% for k = 2:Ny - 1  
%     for j = 2:Nx - 1
...
% end
% 
% % Mapping B-Vector to grid to see if it is right
% BM = reshape(B,Ny,Nx);
...
%% Check
% BM = reshape(B,Ny,Nx);
% [c,h] = contourm(xcenter,ycenter,BM);
% clegendm(c,h,4);