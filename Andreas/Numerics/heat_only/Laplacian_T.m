...
k = 1;
m = vector_index(j,k);
AP(m)     = 3*(dy/dx^2+dx/dy^2);
% A(m,m)    = AP(m);
% A(m,m+Ny) = AE(m);
% A(m,m+1)  = AN(m);
b_T(m) = b_T(m) - (2*dy/dx^2*BC.T.left(k) + 2*dx/dy^2*BC.T.bottom(j));
...
% A(m,m-Ny) = AW(m);
% A(m,m+1 ) = AN(m);
b_T(m) = b_T(m) - (2*dy/dx^2*BC.T.right(k) + 2*dx/dy^2*BC.T.bottom(j));
...
% A(m,m-Ny) = AW(m);
% A(m,m-1 ) = AS(m);
b_T(m) = b_T(m) - (2*dy/dx^2*BC.T.right(k) + 2*dx/dy^2*BC.T.top(j));
...
% A(m,m+Ny) = AE(m);
% A(m,m-1 ) = AS(m);
b_T(m) = b_T(m) - (2*dy/dx^2*BC.T.left(k) + 2*dx/dy^2*BC.T.top(j));
