...
% Pressure correction method
    ...
    rhs_u = u(:) - dt*c_u(:) - dt * dp_dx(:) - dt/Re*b_u ; 
    rhs_v = v(:) - dt*c_v(:) - dt * dp_dy(:) - dt/Re*b_v + dt * eyTM(:)  ; 
    ...
    p_corr = dLHS_p \ rhs_p(:) ;
    p_corr = reshape(p_corr, Nx, Ny);
     
    % Step 3: Correct velocity field
    u = u - dt * diff(p_corr,1,2)/dx ;
    v = v - dt * diff(p_corr,1,1)/dy ;
...
exportgraphics(gcf,'streamlines_pc.pdf','ContentType','vector')
...
exportgraphics(gcf,'temperature_pc.pdf','ContentType','vector')
