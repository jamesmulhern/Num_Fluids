...
#p_np1
LHS_p_line = Laplacian_p(Nx,Ny,dx,dy)
...
dLHS_p_line = sp.linalg.splu(sp.csc_matrix(LHS_p_line))

#### Applying Pressure Correction Method
...
for t in np.arange(0,t_end,dt):
    ...
    p_n_E = p_n[Ny:]
    p_n_W = p_n[:-Ny]
    dpn_dx = 1 / dx * (p_n_E - p_n_W)

    p_n_N = np.delete(p_n, list(np.linspace(0, len(p_n) - Ny, Nx).astype("int")))
    p_n_S = np.delete(p_n, list(np.linspace(Ny - 1, len(p_n) - 1, Nx).astype("int")))
    dpn_dy = 1 / dy * (p_n_N - p_n_S)

    #RHS u_star
    u_star_convective_term = u_conv(u_n,v_n,dx,dy)
    u_n_lim = u_n[Ny:-Ny]  #removing BC from v so that the size of u_n matches u_star
    RHS_u_star = - 1/(np.sqrt(Gr)) * b_u_star + 1/dt * u_n_lim - u_star_convective_term - dpn_dx
    #solve u_star
    ...
    RHS_v_star = - 1/(np.sqrt(Gr)) * b_v_star + 1/dt * v_n_lim - v_star_convective_term + v_T_term - dpn_dy
    #solve v_star
    ...
    RHS_p_line = 1/dt * div_vec_u_star
    #solve pressure for p'
    p_line = dLHS_p_line.solve(RHS_p_line)

    #computing corrections u'=u_line and v'=v_line
    p_line_E = p_line[Ny:]
    p_line_W = p_line[:-Ny]
    u_line = - dt/dx*(p_line_E-p_line_W) #maybe missing a rho?

    p_line_N = np.delete(p_line, list(np.linspace(0,len(p_line)-Ny,Nx).astype("int")))
    p_line_S = np.delete(p_line, list(np.linspace(Ny-1,len(p_line)-1,Nx).astype("int")))
    v_line = - dt/dy*(p_line_N-p_line_S)
